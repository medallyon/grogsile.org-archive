...
function resetLocals(req, res, next)
{
    ...
    let webApp = webApps["bot.grogsile.org"];
    ...
    if (locals.user)
    {
        fs.readJson(join(__data, "users", req.user.id, "account.json")).then(function(account)
        {
            locals.account = account;

            if (locals.user.guilds)
            {
                locals.user.guilds = locals.user.guilds.filter(g => {
                    let permissions = new Discord.Permissions(g.permissions);
                    return (permissions.has("MANAGE_GUILD", true) || g.owner.id === locals.user.id);
                });

                locals.user.guilds = locals.user.guilds.map(g => {
                    ...
                    return Object.assign(g, {
                        ...
                        channels: guild.channels.map(c => {
                            return {
                                ...
                                children: (c.type === "category") ? c.children.map(x => {
                                    return {
                                        id: x.id,
                                        name: x.name,
                                        type: x.type,
...
router.get("/guilds", middleware.isLoggedIn, resetLocals, function(req, res)
{
...
router.post("/guilds/:id/welcomeMessage", middleware.isLoggedIn, resetLocals, require(join(__dirname, "configHandlers", "welcomeMessage.js")), resetLocals, successfulConfigRequest);
router.post("/guilds/:id/esoNews", middleware.isLoggedIn, resetLocals, require(join(__dirname, "configHandlers", "esoNews.js")), resetLocals, successfulConfigRequest);
router.post("/guilds/:id/esoPatchNotes", middleware.isLoggedIn, resetLocals, require(join(__dirname, "configHandlers", "esoPatchNotes.js")), resetLocals, successfulConfigRequest);
router.post("/guilds/:id/esoYouTube", middleware.isLoggedIn, resetLocals, require(join(__dirname, "configHandlers", "esoYouTube.js")), resetLocals, successfulConfigRequest);
router.post("/guilds/:id/liveServerStatus", middleware.isLoggedIn, resetLocals, require(join(__dirname, "configHandlers", "liveServerStatus.js")), resetLocals, successfulConfigRequest);
// router.post("/guilds/:id/rss", middleware.isLoggedIn, resetLocals, require(join(__dirname, "configHandlers", "rss.js")), resetLocals, successfulConfigRequest);

...
class ESOI extends EventEmitter
{
    constructor(grogClient, esoGuildId)
    {
        ...
        this.client = grogClient;
        this.guild = grogClient.guilds.get(esoGuildId);
        this.guild.webListener = this;

        this.defaultGuildWelcomeMessage = this.guild.config.guild.welcomeMessage.message;
        ...
        dClient.libs.fs.readdir(dClient.libs.join(__lib, "handlers", "eso")).then(function(handlers)
        {
            ...
            {
                self.on(handler.replace(".js", ""), require(dClient.libs.join(__lib, "handlers", "eso", handler)));
            }
        }).catch(console.error);
    }
    ...
    setServerMaintenance(active)
    {
        if ((typeof active) !== "boolean")
            throw new TypeError("{active} must be a boolean value");

        if (active)
            this.guild.config.guild.welcomeMessage.message += "\nESO Maintenance is currently on-going!";
        else
            this.guild.config.guild.welcomeMessage.message = this.defaultGuildWelcomeMessage;

        this.maintenance = active;
        this.client.persistentVars.eso.maintenance = this.maintenance;
        this.client.persistentVars.save().catch(this.client.modules.utils.consoleError);

        this.client.modules.commands.changeActivity();
    }
}

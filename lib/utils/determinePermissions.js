...
function determinePermissions(member)
{
    if (!member) return console.error(new Error("'member' cannot be null"));
    if (!(member instanceof Discord.GuildMember)) return console.error(new Error("'member' must be an instance of 'Discord.GuildMember'"));

    // if member is inside a server
    if (member.guild)
    {
        // if member is the bot owner/creator (me)
        if (member.id === dClient.config.bot.discord.ownerId)
            return 1000;

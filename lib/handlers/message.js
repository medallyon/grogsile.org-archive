...
function message(msg)
{
    ...
    if (msg.command)
    {
        ...
        {
            const command = dClient.modules.commands[cmd];
            ...
            for (const alias of command.alias)
            {
                ...
                if (msg.command === alias)
                {
                    ...
                    let guildCommands = msg.guild.config.guild.commands;
                    if (guildCommands[cmd].enabled)
                    {
                        // check if the member actually has permission to execute the command
                        if (dClient.modules.utils.hasPermission(command, msg.member))
                        {
                            try
                            {
                            ...
                            } catch (err)
                            {
                                ...
                                dClient.modules.utils.consoleError(err);
                            ...
                            {
                                if (!stats.commands.hasOwnProperty(cmd)) stats.commands[cmd] = 0;
                                stats.commands[cmd]++;
    ...
    if (/\[.+\]/g.test(msg.content) && !/`.*?\[.+\].*?`/g.test(msg.content))
    {
        ...
        dClient.modules.commands.esoItem(msg, requestedItem);

        // amend statistics
        dClient.libs.fs.readJson(dClient.libs.join(__data, "statistics.json")).then(function(stats)
        {
            if (!stats.commands.hasOwnProperty("esoItem_inline")) stats.commands["esoItem_inline"] = 0;

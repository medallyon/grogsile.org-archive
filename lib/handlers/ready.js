...
function activateListeners()
{
    for (const file of dClient.libs.fs.readdirSync(dClient.libs.join(__lib, "handlers")))
    {
        ...
        else
        {
            ...
            {
                if (!(dClient[file] instanceof EventEmitter)) dClient[file] = new EventEmitter();
...
function ready()
{
    ...
    // generate a new secret every week
    new CronJob("0 0 0 */7 * *", function()
    {
        dClient.config.web.session = { secret: dClient.modules.utils.genSecret() };
        dClient.libs.fs.outputJsonSync(dClient.libs.join(__webdir, "config.json"), dClient.config.web);
    }, null, true, "UTC");

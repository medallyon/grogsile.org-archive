function characterEdit(oldChar, newChar)
{
    ...
    {
        if (!member.roles.has(allianceId)) dClient.eso.addMemberToAlliance(member, allianceId).catch(console.error);
        ...
        {
            dClient.setTimeout(() => { dClient.eso.removeMemberFromAlliance(member, dClient.constants.discord.esoi.roles[oldChar.alliance]).catch(console.error) }, 1000 * 10);
        }
    }).catch(console.error);
}

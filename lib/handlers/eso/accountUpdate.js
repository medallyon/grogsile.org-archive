...
function accountUpdate(oldAccount, newAccount)
{
    ...
    else
    {
        ...
        {
            if (newAccount.server && !member.roles.has(dClient.constants.discord.esoi.roles[newAccount.server])) rolesToAdd.push(dClient.constants.discord.esoi.roles[newAccount.server]);
            if (oldAccount.server && member.roles.has(dClient.constants.discord.esoi.roles[oldAccount.server])) rolesToRemove.push(dClient.constants.discord.esoi.roles[oldAccount.server]);
        }

        if (oldAccount.platform !== newAccount.platform)
        {
            if (newAccount.platform && !member.roles.has(dClient.constants.discord.esoi.roles[newAccount.platform])) rolesToAdd.push(dClient.constants.discord.esoi.roles[newAccount.platform]);
            if (oldAccount.platform && member.roles.has(dClient.constants.discord.esoi.roles[oldAccount.platform])) rolesToRemove.push(dClient.constants.discord.esoi.roles[oldAccount.platform]);
        }
        ...
        {
            if (!member.roles.has(dClient.constants.discord.esoi.roles["Server Updates"])) rolesToAdd.push(dClient.constants.discord.esoi.roles["Server Updates"]);
        }

        else
        {
            if (member.roles.has(dClient.constants.discord.esoi.roles["Server Updates"])) rolesToRemove.push(dClient.constants.discord.esoi.roles["Server Updates"]);
        }
    }

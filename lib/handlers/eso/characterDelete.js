function characterDelete(character)
{
    ...
    {
        if (characters.every(c => c.alliance !== character.alliance) && member.roles.has(allianceId))
            dClient.eso.removeMemberFromAlliance(member, allianceId).catch(console.error);
    }).catch(console.error);
}

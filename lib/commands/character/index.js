    ...
    dClient.eso.fetchAccount(user).then(function(account)
    {
        dClient.eso.fetchCharacters(user).then(function(characters)
        {
            ...
            if (!character)
            {
                ...
                {
                    finalEmbed.addField(`${char.characterName} (${char.champion ? ("CP" + account.level) : ("lvl " + char.level)})`, `${dClient.constants.discord.esoi.emojis.class[char.class.toLowerCase().split(" ").join("")].emoji}${char.class}\n${dClient.constants.discord.esoi.emojis.alliance[char.alliance].emoji}${char.alliance}\n${dClient.constants.discord.esoi.emojis.race[char.race.toLowerCase()].emoji}${char.race}\n${char.roles.map(r => dClient.constants.discord.esoi.emojis.role[r.toLowerCase()].emoji).join(" ")}`, true);
                }

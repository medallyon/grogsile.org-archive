...
function treatDescription(text)
{
    ...
    if (textArray.length > 150) text = textArray.slice(0, 150).join(" ");
...
function constructEmbed(video)
{
    ...
    let e = new Discord.MessageEmbed(dClient.modules.utils.createEmptyRichEmbedObject())
        ...
        .setFooter(`${dClient.constants.discord.embed.footer.text} | ${dClient.modules.utils.fancyESODate(new Date(video.publishedAt))}`, dClient.constants.discord.embed.footer.icon_url);
    ...
    for (const guild of dClient.guilds.values())
    {
        if (guild.config.eso.youtube.enabled)
        {
            ...
            dClient.modules.utils.toggleRoles(guild.config.eso.youtube.toggleRoles, guild.config.eso.youtube.roles.map(x => guild.roles.get(x))).then(function(roles)
            {
                ...
                {
                    dClient.modules.utils.toggleRoles(guild.config.eso.youtube.toggleRoles, guild.config.eso.youtube.roles.map(x => guild.roles.get(x))).catch(console.error);
                }).catch(console.error);
            }).catch(console.error);
        }
    }
    ...
    fetchNewestVideo().then(function(newestVideo)
    {
        dClient.libs.fs.readJson(dClient.libs.join(__dirname, "savedVars.json")).then(function(previousVideo)
        {
            if (videoIsESO(newestVideo, previousVideo))
            {
                ...
                dClient.libs.fs.outputJson(dClient.libs.join(__dirname, "savedVars.json"), newestVideo).catch(console.error);
            }
        }).catch(console.error);
    }).catch(console.error);
}

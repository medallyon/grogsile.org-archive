...
function structureEmbed(status, platform = null)
{
    ...
    if (!platform || platform === "pc")
    {
        ...
        for (const server in status["PC"])
        {
            ...
            if (!status["PC"][server] && (now.getUTCDay() === 1 && (now.getUTCHours() >= 8 && now.getUTCHours() <= 16))) e.setDescription("💔 Is it Morndas already? Must be Maintenance. :tools:");
    ...
    if (!platform || platform === "ps4" || platform === "consoles")
    {
        ...
        for (const server in status["PS4"])
        {
            ...
            if (!status["PS4"][server] && (now.getUTCDay() === 2 && (now.getUTCHours() >= 10 && now.getUTCHours() <= 18))) e.setDescription("💔 Is it Tirdas already? Must be Maintenance. :tools:");
    ...
    if (!platform || platform === "xbone" || platform === "consoles")
    {
        ...
        for (const server in status["XBONE"])
        {
            ...
            if (!status["XBONE"][server] && (now.getUTCDay() === 3 && (now.getUTCHours() >= 10 && now.getUTCHours() <= 18))) e.setDescription("💔 Is it Middas already? Must be Maintenance. :tools:");

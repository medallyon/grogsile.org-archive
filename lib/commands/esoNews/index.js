...
{
    for (let guild of dClient.guilds.filter(g => g.id === "236925155442425866").values())
    {
    ...
    else
    {
        dClient.libs.fs.readJson(dClient.libs.join(__dirname, "savedVars.json")).then(function(savedVars)
        {
            if (savedVars.length > 1)
            {
                ...
                dClient.modules.utils.createSelectorEmbed(msg, savedVars.map(function(x)
                {
                    return {
                        name: x.title,
                        value: `[Read this article online](${x.link}) | ${dClient.modules.utils.fancyESODate(new Date(x.pubDate))}`
                    };
                }), selectorEmbed).then(function([selectedNumber, collectorMessage])
                {
